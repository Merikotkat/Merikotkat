class VisitationFormsController < ApplicationController
  ...
  def index
    ...
    if (defined? params[:type] and !params[:type].nil?)
      ...
      if (@visitation_forms = VisitationForm.get_forms_of_type @user, params[:type], params[:sortby], params[:order], @nestid, @speciesid, @datefrom, @dateto, @nestname) == false
        not_found
      end
    else
  ...
  def visitation_form_params
    params.require(:visitation_form).permit(:photographer_name, :visit_date, :camera, :lens, :teleconverter, :municipality, :nest, :nest_id, :photographer_id, :species_id, :additional_info, :gender_determination_method, :ringed)
  end
end
